        ...
        if env == 2:  # Windows
            ...
            for line in ips:  # Determine ipv4 address
                if 'IPv4 Address' in line.decode():
                    ...
                    if ip != "127.0.0.1":
                        print("* Trying to run ssl server at https://%s:%s/\n" % (ip, port))
                        os.system("python %smanage.py runsslserver %s:%s" % (os.path.realpath(__file__).strip('runssl.py'), ip, port))  # Run the command to start the server

        elif env == 1:  # Unix
            ip = net.strip().split(b'inet addr:')[1].strip().split(b' ')[0]  # Extract the IP address
            if ip != "127.0.0.1":
                print("* Trying to run ssl server at https://%s:%s/\n" % (ip, port))
                os.system("python manage.py runsslserver %s:%s" % (os.path.realpath(__file__).strip('runssl.py'), ip, port))  # Run the command to start the server
